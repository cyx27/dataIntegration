...
from sklearn.ensemble import RandomForestClassifier  # 修改1：导入随机森林分类器
...
# 定义并训练随机森林模型
rf = RandomForestClassifier(n_estimators=100)  # 修改2：使用随机森林分类器，同时设置决策树个数为100
...
y_pred = rf.predict(X_test)  # 修改3：使用随机森林模型进行预测
df_test = pd.DataFrame({'uid': uid_test, 'star_level': y_pred})
df_test.to_csv('predicted_star_level_info_random_forest.csv', index=False)
