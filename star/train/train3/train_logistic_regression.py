...
# 将特征与标签拆分为X和y
X_train = df_train[['avg_mth', 'avg_year', 'total_tran_amt', 'tran_cnt', 'is_shareholder', 'is_black', 'is_contact', 'is_mgr_dep']].replace({'Y': True, 'N': False}).values
y_train = df_train['star_level'].values
# 将测试集中的star_level设置为-1以进行预测
X_test = df_test[['avg_mth', 'avg_year', 'total_tran_amt', 'tran_cnt', 'is_shareholder', 'is_black', 'is_contact', 'is_mgr_dep']].replace({'Y': True, 'N': False}).values
...
df_test.to_csv('predicted_star_level_info_logistic_regression.csv', index=False)